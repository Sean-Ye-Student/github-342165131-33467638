...
def Lazers():
    ...
    for ii, lazer in enumerate(lazers):
        ...
        if closest_index > -1:
            ...
            if parent[asteroid_scale] == 1:
                    score += 20
            elif parent[asteroid_scale] == 0.5:
                score += 50
            else:
                score += 100
                break
...
def draw():
    ...
    if mode == 3:
        ...
        if smoothTrans > 150:
            ...
            previous_score = int(highScore.read())
            highScore.close()
            highScore = open("HighScore.txt", "w")
            new_highScore = max(score, previous_score)
            print(new_highScore)
            highScore.write(str(new_highScore))
            highScore.close()
            text("HIGH SCORE:", 320, 275) 
            text(str(new_highScore), 440, 350)
            print()
